...
def chat():
    ...
    if not conversation['data_collection_complete']:
        ...
        # Auto-trigger analysis if conversation is getting long or AI says ready
        if "READY_FOR_ANALYSIS" in ai_response or len(conversation['messages']) >= 40:
            # Data collection complete, switch to analysis
            ...
            # STAGE 2: Medical Analysis with II-Medical-8B-1706
            analysis = analyze_with_medical_model(patient_data)
            
            ai_response = f"""**📋 SOAP NOTE - AUTOMATIC DATA COLLECTION COMPLETED**
...
**📝 CLINICAL NOTE COMPLETED (Automatic):**
...
---
*SOAP note completed automatically. You can create a new consultation using the sidebar.*"""
            
            # Update the last assistant message with final response
            conversation['messages'][-1] = {"role": "assistant", "content": ai_response}
...
def manual_analysis():
    ...
    # Check if there are enough messages for analysis
    if len(conversation['messages']) < 4:
        return jsonify({'error': 'Insufficient data for analysis. Need at least 2 exchanges.'}), 400
